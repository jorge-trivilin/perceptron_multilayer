...
class PerceptronMultilayer:
    def __init__(
        self, input_size, hidden_size, output_size, learning_rate=0.1, logger=None
    ):
        """
        ...
        self.bias_hidden = np.random.randn(self.hidden_size) * np.sqrt(
            1.0 / self.input_size
        )
        self.bias_output = np.random.randn(self.output_size) * np.sqrt(
