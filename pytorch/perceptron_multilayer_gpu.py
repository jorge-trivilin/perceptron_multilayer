...
class PerceptronMultilayer:
    def __init__(self,
        ...
        self.weights_input_hidden = torch.randn(self.hidden_size, self.input_size, device=self.device) * np.sqrt(1. / self.input_size)
        self.weights_hidden_output = torch.randn(self.output_size, self.hidden_size, device=self.device) * np.sqrt(1. / self.hidden_size)
        self.bias_hidden = torch.randn(self.hidden_size, device=self.device) * np.sqrt(1. / self.input_size)
        self.bias_output = torch.randn(self.output_size, device=self.device) * np.sqrt(1. / self.hidden_size)
    ...
    def binary_cross_entropy(self, expected_output, y_output):
        ...
        bce = - expected_output * torch.log(y_output) + (1 - expected_output) * torch.log(1 - y_output)
    ...
    def backpropagation(self, error, inputs, y_hidden, y_output):
        ...
        delta_hidden = torch.matmul(self.weights_hidden_output.T, delta_output) * self.sigmoid_derivative(y_hidden)

        # Atualização dos pesos e bias
        self.weights_hidden_output += self.learning_rate * torch.outer(delta_output, y_hidden)
        self.weights_input_hidden += self.learning_rate * torch.outer(delta_hidden, inputs)
