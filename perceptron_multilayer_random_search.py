...
class PerceptronMultilayer:
    def __init__(
        ...
        self.logger.info(
            f"Initializing with input_size={input_size}, hidden_size={hidden_size}, output_size={output_size}, learning_rate={learning_rate}, random_seed={random_seed}"
        )
        ...
        self.weights_input_hidden = self.rng.normal(
            0, np.sqrt(1.0 / self.input_size), (self.hidden_size, self.input_size)
        )
        self.weights_hidden_output = self.rng.normal(
            0, np.sqrt(1.0 / self.hidden_size), (self.output_size, self.hidden_size)
        )
        self.bias_hidden = self.rng.normal(
            0, np.sqrt(1.0 / self.input_size), (self.hidden_size,)
        )
        self.bias_output = self.rng.normal(
            0, np.sqrt(1.0 / self.hidden_size), (self.output_size,)
        )
    ...
    def binary_cross_entropy(self, expected_output, y_output):
        ...
        bce = -expected_output * np.log(y_output) + (1 - expected_output) * np.log(
            1 - y_output
        )
        return np.mean(bce)
    ...
    def backpropagation(self, error, inputs, y_hidden, y_output):
        ...
        delta_hidden = np.dot(
            delta_output, self.weights_hidden_output
        ) * self.sigmoid_derivative(y_hidden)

        # Atualizar pesos da camada de saída
        self.weights_hidden_output += (
            self.learning_rate * np.dot(delta_output.T, y_hidden) / batch_size
        )

        # Atualizar pesos da camada oculta
        self.weights_input_hidden += (
            self.learning_rate * np.dot(delta_hidden.T, inputs) / batch_size
        )
